...
SROUTER_DECLARED_NAME(keySRouterRemote_Target, @"SRR_Target"); //目标对象
SROUTER_DECLARED_NAME(keySRouterRemote_Params, @"SRR_Params"); //参数

SROUTER_DECLARED_NAME(keySRouterRemoteAction_Push, @"push");
SROUTER_DECLARED_NAME(keySrouterRemoteAction_Present, @"present");
...
- (BOOL)handlerCommandWithURL:(NSURL *)url completion:(void (^)(NSDictionary *))completion
{
    ...
    if (completion) {
        ...
        SROUTER_REGISETER_BLOCK(targetClass, ^(NSDictionary *dict){
            if (completion) {
                completion(dict);
            }
        });
